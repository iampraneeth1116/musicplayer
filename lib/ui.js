...
function render(model) {
    ...
    if (model.repeat !== 'off') badges.push(`repeat ${model.repeat}`);
    ...
    const barWidth = Math.max(10, width - times.length - volumeText.length - 5);
    ...
    if (model.filterMode) {
        ...
        const help = ' ↑↓ move · ⏎ play · space pause · x stop · n/b next/prev · ←→ seek · +/- vol · s shuffle · r repeat · / filter · q quit';
