...
// Arrow keys arrive as ESC [ A/B/C/D; this maps that last letter to a name.
const CSI_FINAL = {
    0x41: 'up',
    0x42: 'down',
    0x43: 'right',
...
function decode(buffer) {
    ...
    while (i < buffer.length) {
        ...
        if (byte === 0x1b) {
            if (buffer[i + 1] === 0x5b) {
                // Skip any numbers in the middle (Page Up sends ESC [ 5 ~), so keys
                // the player doesn't use are skipped whole instead of read as letters.
                let j = i + 2;
